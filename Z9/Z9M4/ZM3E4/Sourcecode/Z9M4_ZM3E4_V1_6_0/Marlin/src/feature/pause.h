...
typedef struct {
  float unload_length, load_length;
	float slow_unload_length, slow_load_length;
...
const uint8_t &mix_multiplier=1 DXC_PARAMS);
...
void resume_print(const float &slow_load_length=0, const float &fast_load_length=0, const float &extrude_length=ADVANCED_PAUSE_PURGE_LENGTH, 
	const int8_t max_beep_count=0, const uint8_t &mix_multiplier=1, int16_t targetTemp=0 DXC_PARAMS);

bool load_filament(const float &slow_load_length=0, const float &fast_load_length=0, const float &extrude_length=0, 
	const int8_t max_beep_count=0, const bool show_lcd=false,
	const bool pause_for_user=false, const PauseMode mode=PAUSE_MODE_PAUSE_PRINT, const uint8_t &mix_multiplier=1 DXC_PARAMS);
