...
static bool ensure_safe_temperature(const bool wait=true, const PauseMode mode=PAUSE_MODE_SAME) {
  ...
  TERN_(HAS_LCD_MENU, lcd_pause_show_message(PAUSE_MESSAGE_HEATING, mode));
	TERN_(HAS_DWIN_LCD, DWIN_Pause_Show_Message(PAUSE_MESSAGE_HEATING, mode));
...
bool load_filament(const float &slow_load_length/*=0*/, const float &fast_load_length/*=0*/, const float &purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
                   ...
                   const uint8_t &mix_multiplier/*=1*/
                   DXC_ARGS
) {
  ...
  // Slow Load filament
  if (slow_load_length) unscaled_e_move(slow_load_length * mix_multiplier, FILAMENT_CHANGE_SLOW_LOAD_FEEDRATE * mix_multiplier);

  // Fast Load Filament
  if (fast_load_length) {
    #if FILAMENT_CHANGE_FAST_LOAD_ACCEL > 0
      const float saved_acceleration = planner.settings.retract_acceleration;
      planner.settings.retract_acceleration = FILAMENT_CHANGE_FAST_LOAD_ACCEL*mix_multiplier;
    #endif

    unscaled_e_move(fast_load_length*mix_multiplier, FILAMENT_CHANGE_FAST_LOAD_FEEDRATE * mix_multiplier);
    ...
    for (float purge_count = purge_length; purge_count > 0 && wait_for_user; --purge_count)
      unscaled_e_move(mix_multiplier, ADVANCED_PAUSE_PURGE_FEEDRATE * mix_multiplier);
    wait_for_user = false;
  #else		
    do {
      if (purge_length > 0) {
        ...
        unscaled_e_move(purge_length*mix_multiplier, ADVANCED_PAUSE_PURGE_FEEDRATE * mix_multiplier);
      }
...
bool unload_filament(const float &unload_length, const bool show_lcd/*=false*/,
  ...
  // Unload filament
    ...
    const float saved_acceleration = planner.settings.retract_acceleration;
    planner.settings.retract_acceleration = FILAMENT_CHANGE_UNLOAD_ACCEL*mix_multiplier;
  #endif
...
bool pause_print(const float &retract, const xyz_pos_t &park_point, const float &unload_length/*=0*/, const bool show_lcd/*=false*/ , const uint8_t &mix_multiplier/*=1*/ DXC_ARGS ) {
  ...
  if (retract && thermalManager.hotEnoughToExtrude(active_extruder)) {
    DEBUG_ECHOLNPAIR("... retract:", retract);				
		unscaled_e_move(retract*mix_multiplier, PAUSE_PARK_RETRACT_FEEDRATE*mix_multiplier);		 
  }
  ...
  if (unload_length)   // Unload the filament
		unload_filament(unload_length, show_lcd, PAUSE_MODE_CHANGE_FILAMENT, mix_multiplier);	
  #if ENABLED(DUAL_X_CARRIAGE)
    active_extruder = saved_ext;
    extruder_duplication_enabled = saved_ext_dup_mode;
    stepper.set_directions();
...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  while (wait_for_user) {
    ...
    if (nozzle_timed_out) {
      ...
      ensure_safe_temperature(false, PAUSE_MODE_PAUSE_PRINT);
 ...
 */
void resume_print(const float &slow_load_length/*=0*/, const float &fast_load_length/*=0*/, const float &purge_length/*=ADVANCED_PAUSE_PURGE_LENGTH*/, const int8_t max_beep_count/*=0*/, const uint8_t &mix_multiplier/*=1*/, int16_t targetTemp/*=0*/ DXC_ARGS) {
  ...
  // Load the new filament
	load_filament(slow_load_length, fast_load_length, purge_length, max_beep_count, true, nozzle_timed_out, PAUSE_MODE_SAME, mix_multiplier DXC_PASS);
  ...
  // Retract to prevent oozing
  unscaled_e_move(-(PAUSE_PARK_RETRACT_LENGTH*mix_multiplier), feedRate_t(PAUSE_PARK_RETRACT_FEEDRATE*mix_multiplier));
  ...
  // Intelligent resuming
    ...
    if (fwretract.retracted[active_extruder])
      unscaled_e_move(-fwretract.settings.retract_length*mix_multiplier, fwretract.settings.retract_feedrate_mm_s*mix_multiplier);
  #endif

  // If resume_position is negative
  if (resume_position.e < 0) unscaled_e_move(resume_position.e*mix_multiplier, feedRate_t(PAUSE_PARK_RETRACT_FEEDRATE*mix_multiplier));
  #if ADVANCED_PAUSE_RESUME_PRIME != 0
    unscaled_e_move(ADVANCED_PAUSE_RESUME_PRIME*mix_multiplier, feedRate_t(ADVANCED_PAUSE_PURGE_FEEDRATE*mix_multiplier));
  #endif
